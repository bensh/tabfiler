...
export const DEFAULT_SETTINGS = {
  ...
  autoCategories: [],
  grace: true,             // give a chance to cancel a save when closing a tab
  graceMode: "undo",       // "undo" (save then allow remove) | "delay" (hold, then save)
  graceSeconds: 10,        // how long the cancel window lasts
  onboarded: false,
};
...
export function validateBackup(obj) {
  ...
  const boolKeys = ["autoOnLoad", "autoOnClose", "notifyOnAuto", "askWhenNoFit", "wholeWord", "grace", "onboarded"];
  ...
  if (["undo", "delay"].includes(s.graceMode)) settings.graceMode = s.graceMode;
  if (Number.isFinite(s.graceSeconds)) settings.graceSeconds = Math.max(3, Math.min(60, Math.round(s.graceSeconds)));
