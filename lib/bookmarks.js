...
export async function resolveDuplicate(api, url, title) {
  const byUrl = await api.bookmarks.search({ url });
  const urlHit = byUrl.find((h) => h.url === url);
  ...
  const titleHits = byTitle.filter((h) => h.url && normTitle(h.title) === norm);
...
export async function fileBookmark(api, { tab, categoryName, rootFolderName, duplicates }) {
  ...
  const match = await resolveDuplicate(api, tab.url, tab.title);
